  ...
  return {
    start: {
      ...
      maxValue: helpers.withMessage(
        "The start field must be less than the end field",
        maxValue(props.form.hourly_rate.end - 1)
      ),
    },
    end: {
      ...
      minValue: helpers.withMessage(
        ...
        minValue(props.form.hourly_rate.start)
      ),
    },
  };
});

const v$ = useVuelidate(rules, props.form.hourly_rate);
</script>
...
<template>
  <div class="p-6 flex flex-col bg-white rounded-md">
    ...
    >
      <div
        class="bg-red-400 p-3 flex gap-2 items-start text-white rounded"
        v-if="v$.start.$error"
      >
        <Icon name="zondicons:exclamation-solid" class="text-lg" />
        <p
          class="text-xs"
          v-for="(error, index) in v$.start.$errors"
          :key="index"
        >
          {{ error.$message }}
        </p>
    ...
    <div class="flex gap-3">
      <FormElementsInput
        name="start"
        v-model="form.hourly_rate.start"
        type="number"
        placeholder="Min"
        label="Min"
        icon="healthicons:dollar"
        iconClass="absolute top-3 right-3 h-full text-xl text-[--gray-color]"
        @touch="v$.start.$touch"
      />

      <FormElementsInput
        v-model="form.hourly_rate.end"
        type="number"
        placeholder="Max"
        label="Max"
        icon="healthicons:dollar"
