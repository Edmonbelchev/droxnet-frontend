  ...
  if (response.status.value === "success") {
    const result = response.data.value.data.map((award: any) => {
      return {
        ...
        title: award.title,
        date: award.date,
        files: award.files.map((file: any) => {
          return file.url;
        }),
        opened: false,
      };
    ...
    // Update the award store
    awardStore.storeValues(result);
...
const addAward = () => {
  ...
  // Update the award store
  awardStore.storeValues(props.form);
};
...
onMounted(async () => {
  // If award store have value, load the data from it
  if (awardStore.awards.length > 0) {
    emit("update", awardStore.awards);
    emit("loaded", true);
  } else {
    await retrieveUserAwards();
  }
        ...
        >
          <div class="flex gap-2">
            ...
            <FormElementsInput
              ...
              name="date"
              :error="v$[index].date.$error"
              :valid="!v$[index].date.$invalid && item.date.length > 0"
              :errorMessages="v$[index].date.$errors"
              @touch="v$[index].date.$touch"
            />
          </div>
